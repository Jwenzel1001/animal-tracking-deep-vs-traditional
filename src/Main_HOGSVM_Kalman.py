...
import cv2
import numpy as np
from HOGSVM_Tracker import HOGSVMDetector
from Kalman_Filter import SimpleKalmanTracker
...
detector = HOGSVMDetector(
    model_path=r"C:\Users\PC\Desktop\School\Graduate_School\Spring_2025\CPRE575\AirSim_Project_Code_Snippets\Wolf_Data\Vision_Pipeline\Traditional_Method\HOGSVM\HOGSVM_Models\hogsvm_C0.01_patch64x144_cell8x8.pkl",
    window_size=(64, 144),
    step_size=32,
    scale_factor=1.25,
    score_threshold=2
...
while True:
    ...
    # Run HOG+SVM detection every specified frames
    if frame_idx % 1 == 0:
        detections = detector.detect(frame)
        print(f"[Frame {frame_idx}] HOG+SVM detections: {len(detections)}")
    else:
        detections = []
    ...
    # === Display the frame ===
    cv2.imshow('AirSim Kalman Tracking View (HOG+SVM)', frame)
...
# === Print final metrics ===
metrics.print_summary("HOGSVM_Kalman_Output.txt")